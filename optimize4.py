## Crazy thought. Since the vowels are easy to guess, if you know the consonants,
## you can totally find the rest, right? right?
## Only time will tell...
...
def evalTheString(comb):
	...
	vowelLessString = uniqueString.translate({ord(i): None for i in 'aeiou'})
	print(vowelLessString)
	if(len(vowelLessString) > 18):
		f.write(str(comb) + "," + str(len(uniqueString)) + "\n")
		print(currentString)
...
with open("words_lawl.txt") as file:
	for line in file:
		if(not(line.find("(") > -1  or line.find("/") > -1 or line.find("-") > -1 or line.find("0") > -1)):
...
# for i in range(len(wordslist)*300*300*300*300):
# 	## find a rand word
